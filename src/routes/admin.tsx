  ...
  return (
    <div className="container-page flex min-h-[70vh] items-center justify-center">
      <form
        onSubmit={async (e) => {
          e.preventDefault();
          if (!(await login(pwd))) setError(true);
        }}
      ...
      >
        ...
        <button className="btn-base w-full bg-primary text-primary-foreground">
...
function Shell() {
  ...
  const items: {
    ...
    { to: "/admin/apartments", exact: false, label: t("admin.nav.apartments"), Icon: Home },
    {
      ...
      exact: false,
      label: t("admin.nav.availability"),
      Icon: CalendarRange,
    },
  ...
  return (
    <div className="container-page grid gap-8 py-10 md:grid-cols-[220px_1fr]">
      <aside className="space-y-1">
        ...
        <button
          onClick={logout}
          className="mt-4 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-muted-foreground hover:bg-secondary/60"
        >
