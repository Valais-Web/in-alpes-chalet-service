...
- **Réservation transactionnelle** : `submit-booking` vérifie la dispo côté serveur (chevauchement `booked`/`blocked`, capacité vs `maxGuests`) puis insère demande + hold en une transaction (`sql.transaction`) ; renvoie 409/400. La confirmation admin passe la plage en `booked` de façon atomique et refuse un chevauchement. Échecs de publication/email loggés explicitement (jamais avalés).
- **Propriété des holds/booked** : `availability.booking_request_id` relie chaque hold (et la plage `booked` qu'il devient) à sa demande. Une décision admin agit **uniquement sur les lignes de SA demande**, jamais par chevauchement de dates — décliner/confirmer une demande ne touche plus le soft hold d'un autre voyageur (§5). Colonne masquée du JSON public. Migration `db/add-hold-ownership.mjs`.
- **Machine à états des demandes** (`netlify/lib/bookingTransitions.ts`, `resolveDecision(from,to)`) : l'op calendrier (`book`/`unbook`/`release_hold`/`none`) est dérivée serveur du statut **courant + cible**, pas seulement de la cible. Reconfirmer est idempotent (pas de double `booked`) ; `accepté → refusé` **retire** la plage `booked` (plus de réservation fantôme) ; retour à `pending` rejeté (400). L'`action` du client n'est plus qu'un indice.
...
- **Validation** (`netlify/lib/validation.ts`) : dates calendaires réelles, pas d'arrivée passée, bornes lat/lng, images = URLs Cloudinary HTTPS ; `expiresAt` refusé hors `prebooked` ; contraintes CHECK miroir en base. `RETENTION_MONTHS` validé au cold-start prod (nombre positif).
- **Types partagés** : `shared/domain.ts` importé par le client et le serveur (fin de la duplication).

**Limites connues (acceptées, pré-lancement mono-admin)** : la fenêtre lecture-puis-écriture entre la vérif de dispo publique et l'insert du hold reste théorique (le hold n'est que `prebooked` soft ; la contrainte d'exclusion empêche toujours de confirmer un vrai double booking) — non corrigée car le driver Neon HTTP n'offre pas de transaction interactive `FOR UPDATE`. Le throttling de login est best-effort (Blobs, pas de compteur atomique) : une vraie protection brute-force = règle firewall Netlify. CSP autorise encore `'unsafe-inline'` (hydratation SSR ; nonce = durcissement futur).

Migrations DB appliquées : `db/add-booking-integrity.mjs`, `db/add-domain-constraints.mjs`, `db/set-booking-restrict.mjs`, `db/add-hold-ownership.mjs`.
